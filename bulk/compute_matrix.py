...
for turning it into a job matrix and for reading entry backend/model/status, so
a local codex drain and any future CI dispatcher behave identically.

rulespec-ca generation is LOCAL (codex): a local worker selects pending entries
and runs `axiom-encode encode <citation> --apply --backend codex`. There is no
OpenAI-backed GitHub dispatcher here (no OPENAI_API_KEY secret), so `--format
matrix` is retained only for parity/future use; the local path uses
`--format table` / `--get`.

Usage:
  # Human-readable listing of pending entries (optionally capped/filtered):
  python bulk/compute_matrix.py --status pending [--batch A] [--limit 8] --format table

  # Read one field (used by the drain to look up backend/model per entry):
  python bulk/compute_matrix.py --get ca/policy/cra/t1-2025/... --field model
...
def citation_slug(citation: str) -> str:
    ...
    ca/policy/cra/t1-2025/canada-workers-benefit -> ca-policy-cra-t1-2025-canada-workers-benefit
    """
        ...
        out.append(
            {
                ...
                "repo": entry.get("repo", "rulespec-ca"),
...
def main() -> int:
    ...
    if args.format == "count":
        ...
        for item in selected:
            print(f"{item['slug']:44s} {item['backend']}:{item['model']:10s} {item['citation']}")
